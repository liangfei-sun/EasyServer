  ...
  <div class="market-page">
        ...
        <el-row :gutter="16">
          <el-col :span="8" v-for="mod in filteredModules(cat.key)" :key="mod.id" style="margin-bottom: 16px">
            <el-card shadow="hover">
              ...
              <div class="mod-meta">
                <span v-if="mod.port">端口: {{ mod.port }}</span>
    ...
    <el-dialog v-model="configVisible" :title="'安装 ' + installingModule?.name" width="500px">
      <el-form :model="installConfig" label-width="120px">
        <el-form-item v-for="field in configFields" :key="field.name" :label="field.label || field.name">
          <el-input v-if="field.type === 'text' || field.type === 'password'" v-model="installConfig[field.name]" :type="field.type" :placeholder="field.description" />
          <el-input-number v-else-if="field.type === 'number'" v-model="installConfig[field.name]" />
          <el-switch v-else-if="field.type === 'bool'" v-model="installConfig[field.name]" />
          <el-input v-else v-model="installConfig[field.name]" :placeholder="field.description" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const categories = [
  ...
  { key: 'notes', label: '笔记/知识' },
  { key: 'media', label: '媒体娱乐' },
...
const installModule = (mod) => {
  ...
  installConfig.value = {}
  ;(mod.config || []).forEach(f => { installConfig.value[f.name] = f.default || '' })
...
const confirmInstall = async () => {
  ...
  try {
    await api.post(`/api/modules/${installingModule.value.id}/install`, { config: installConfig.value })
...
const uninstallModule = async (mod) => {
  ...
  try {
    await api.post(`/api/modules/${mod.id}/uninstall`)
