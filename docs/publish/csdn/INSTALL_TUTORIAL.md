...
> ❗ **down 提醒**：`docker compose down` 会删除容器并触发配置重新初始化（管理密码等 setup 配置会丢）。跨 down 保留核心配置，可在 override 中加命名卷 `easyserver-app-data:/app/data`（实测有效，详见仓库 `docs/guides/INSTALL_GUIDE.md`「日常启停与数据」步；`/app/.env` 与模块级配置不在此列，down 后重新登录/配置即可）；日常启停用 `stop`/`restart` 最稳妥。
